...
from Wevy.info import (remove_active, is_served_call, joinch)
from Wevy.Data import (get_call, get_dev, get_group, get_channel)
from Wevy.info import (add, db, download, gen_thumb)
...
async def admin_risght(client: Client, message):
  try:
     ...
     if not a.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
      if not message.from_user.id == dev:
       if not message.from_user.username in OWNER:
        return await message.reply_text("**~ لك ياورع انت مو ادمن 🫢 .**")
    ...
    if command == "/pause" or command == "ايقاف مؤقت":
       ...
       button = [[InlineKeyboardButton(text=".~ 𝑬𝒏𝒅", callback_data=f"stop"), InlineKeyboardButton(text="𝑹𝒆𝒔𝒖𝒎𝒆", callback_data=f"resume"), InlineKeyboardButton(text="𝑷𝒂𝒖𝒔𝒆 ~.", callback_data=f"pause")], [InlineKeyboardButton(text=f"ძᥱ᥎ ხ᥆ƚ", url="https://t.me/oYsYY")], [InlineKeyboardButton(text="اضف البوت الي مجموعتك او قناتك ⚡", url=f"https://t.me/{bot_username}?startgroup=True")]]
       await message.reply_photo(photo=img, caption=f"**تم التخطي الى التالي ⏩ **\n\n**🎧 الاسم** : {title}\n**⏱️ المدة** : {dur}\n**🦦 بواسطة البضان** : {requester}", reply_markup=InlineKeyboardMarkup(button))
       try:
           os.remove(file_path)
           os.remove(img)
       except:
