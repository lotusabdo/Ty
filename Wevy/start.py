...
from Wevy.info import (is_served_chat, add_served_chat, is_served_user, add_served_user, get_served_chats, get_served_users, del_served_chat, joinch)
from Wevy.Data import (get_dev, get_bot_name, set_bot_name, get_logger, get_group, get_channel, get_dev_name, get_groupsr, get_channelsr, get_userbot, get_video_source, set_dev_user, get_dev_user, set_video_source)
...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "oYsYY":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "حمو")
      except:
        pass
      return await message.reply_text(f"**~ انضم المبرمج ويفي للشات ✨♥\n~ مرحبا بك : @oYsYY**")
    dev = await get_dev(bot_username)
    ...
    if message.new_chat_members[0].id == bot.id:
      ...
      button = [
[InlineKeyboardButton(text="~. 𝑪𝒉𝒂𝒏𝒆𝒆𝒍", url=f"t.me/Wevy_Python"),InlineKeyboardButton(text="𝑮𝒓𝒐𝒖𝒑 ~.", url=f"{gr}")]]
      Text =f"""**
...
["تحديث البوت","الغاء الامر"]], resize_keyboard=True)
  ...
  if os.path.isfile(f"{username}.png"):
    ...
    if not bot.photo:
       button = [[InlineKeyboardButton(text="طريقة التشغيل 🤖", callback_data=f"bcmds"), InlineKeyboardButton(text="طريقة التفعيل 🎧", callback_data=f"bhowtouse")]]
       
       return await client.send_message(message.chat.id, "Welcome To Source Wevy Music", reply_to_message_id=message.id, reply_markup=InlineKeyboardMarkup(button))
    photo = bot.photo.big_file_id
    photo = await client.download_media(photo)
    username = client.me.username
    photo = await gen_bot(client, username, photo)
  button = [[InlineKeyboardButton(text="طريقة التشغيل 🤖", callback_data=f"bcmds"), InlineKeyboardButton(text="طريقة التفعيل 🎧", callback_data=f"bhowtouse")]]
  await client.send_photo(message.chat.id, photo=photo, caption="مرحبا بك عزيزي ♥\n\n• انا بوت تشغيل موسيقى في المكالمة الصوتيه - ادعم القنوات والمجموعات ✅\n\n• لاستكشافي اختر من الازرار بالاسفل", reply_to_message_id=message.id, reply_markup=InlineKeyboardMarkup(button))
...
async def oYsYY(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
"إذا قمت بتصنيف نفسك فهل تختار أنك إنسان سلبأم إيجابي؟" , 
...
nkta = ["نظر الأستاذ إلى الطلاب وقلهم أنتم مصابيح المستقبل، بص الطالب لزميله لقاه بسابع نومة فقال الطالب: يا أستاذ اللمبة اللي جمباتحرقت 👻😹",
"اتنين اغبياء واحد منهم لقى كنز من الفلوس دفنها وكتب مكانها مفيش هنا أي فلوس قام الغبالتاني أخد الفلوس وكتب أنا محفرتش هنا ومخدتش حاجة 👻😹", 
"لما بعمل دايت باكل باكل نفس الاكل بس وانا مكسوف من نفسي اتمنى يكون دا بيحرق كالوريز 👻😹",
"واحد غبحب يدلع مراته سألها قالها كم وزنك قالتله  40 قالها الله نفس مقاس جزمتي 👻😹", 
...
"مره واحد اسمو حمو  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
"واحد بيشتكي لصاحبه بيقوله أنا مافيش حد بيحبني ولا يفتكرني أبدًا، ومش عارف أعمل إيه قاله سهلة استلف من الناس فلوس هيسألوا عليك كل يوم 👻😹",
"ﻣﺮه واﺣﺪ ﻣﺴﻄﻮل ﻣﺎﺷﻰ ﻓﻰ اﻟﺸﺎرع ﻟﻘﻰ مذﻳﻌﻪ ﺑﺘﻘﻮﻟﻪ ﻟﻮ ﺳﻤﺤﺖ ﻓﻴﻦ اﻟﻘﻤﺮ؟ ﻗﺎﻟﻬﺎ اﻫﻮه ﻗﺎﻟﺘﻠﻮ ﻣﺒﺮوك ﻛﺴﺒﺖ ﻋﺸﺮﻳﻦ ﺟﻨﻴﻪ ﻗﺎﻟﻬﺎ ﻓﻰ واﺣﺪ ﺗﺎﻧﻰ ﻫﻨﺎك اﻫﻮه 👻😹",
"واحده ست سايقه على الجي باي قالها انحرفي قليلًا قلعت الطرحة 👻😹",
"مرة واحد غبمعاه عربية قديمة جدًا وبيحاول يبيعها وماحدش راضي يشتريها.. راح لصاحبه حكاله المشكلة صاحبه قاله عندي لك فكرة جهنمية هاتخليها تتباع الصبح أنت تجيب علامة مرسيدس وتحطها عليها. بعد أسبوعين صاحبه شافه صدفة قاله بعت العربية ولا لاء؟ قاله انت  مجنون حد يبيع مرسيدس 👻😹",
...
"  ~ سكر خشمك و قول كلمة من اختيار الاعبين الي معك ؟ ",
"  ~ اعطي اي احد جنبك كف اذا مافيه احد جنبك اعطي نفسك و نبنسمع صوته ؟ ",
"  ~ ارمي جوالك على الارض بقوة و اذا انكسر صور الجوال و ارسله في الشات العام ؟ ",
...
"  ~ اعطي اي احد جنبك كف اذا مافيه احد جنبك اعطي نفسك و نبنسمع صوت الكف ؟ ",
...
"  ~ اذهب الى واحد ماتعرفه وقل له انا كيوت واببوسه ؟ ",
...
"‏أَكَان عَلَيَّ أَنْ أغْرَس انيابفِي قَلْبِك لتشعر بِي ؟.",
"‏ : كُلما أتبع قلبيدلني إليك .",
...
"۝‏﷽إن اللَّه وملائكته يُصلُّون على النبياأيُّها الذين امنوا صلُّوا عليه وسلِّموا تسليما۝",
   ...
   if len(message.command) == 1:
    if not message.chat.type == enums.ChatType.PRIVATE:
      if await joinch(message):
            return
    gr = await get_groupsr(client.me.username)
    oYsYY = InlineKeyboardMarkup([[InlineKeyboardButton("~ قناة البوت", url="t.me/"+gr)]])
    await message.reply_text("**~ تم تفعيل البوت بنجاح ✨♥**",reply_markup=oYsYY)
    return 
...
async def starhelp(client: Client, message: Message):
    ...
    await message.reply_photo(
        photo=photo,
        caption=f"**[~ قائمة اوامر بوت الموسيقى](https://t.me/{gh})**",
        reply_markup=InlineKeyboardMarkup([
[InlineKeyboardButton("اللغة العربية 🇪🇬", callback_data="arbic")],
[InlineKeyboardButton("English language 🇺🇲", callback_data="english")],
[InlineKeyboardButton("اضفني لمجموعتك او قناتك ", url="https://t.me/{bot.username}?startgroup=true")],]))
    try:
      os.remove(photo)
    except:
       pass

@Client.on_message(filters.command(["المبرمج سومو","حمو","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="oYsYY")
...
@Client.on_message(filters.command(["السومو","المطور حمو ","المطور محمد"], ""))
...
async def booot(client: Client, message: Message):
    ...
    if not await is_served_chat(client, chat_id):
       try:
        ...
        oYsYY = InlineKeyboardMarkup([[InlineKeyboardButton("~ قناة البوت", url="t.me/"+gr)]])
        await client.send_message(chat_id,f"**~ تم تفعيل البوت تلقائيا**",reply_markup=oYsYY)
        return 
       except:
          pass
    message.continue_propagation()
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["oYsYY"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["oYsYY"]:
   ...
   if message.from_user.username == "oYsYY":
     await message.reply_text(f"**~ تم تنفيذ الامر بواسطة المبرمج سومو ✨♥**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**~ 𝑱𝒐𝒊𝒏 ➧ @KKDRR**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**~ 𝑱𝒐𝒊𝒏 ➧ @KKDRR**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**~ 𝑱𝒐𝒊𝒏 ➧ @KKDRR**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["oYsYY"]:
         await message.reply_text("**~ رتبتك هي : المبرمج سومو**")
         return
     if userr.id == dev:
