...
from Wevy.info import (db, add, is_served_call, add_active_video_chat, add_served_call, add_active_chat, gen_thumb, download, remove_active, joinch)
from Wevy.Data import (get_logger, get_userbot, get_call, get_logger_mode, get_group, get_channel)
import asyncio
        ...
        try:
            ...
            if chat.username:
                ...
                try:
                    try:
                       ...
                       if invitelink is None:
                        ...
                        try:
                          ...
                          await client.send_message(chat_id, f"**~ حدث خطأ حاول مرا آخري لاحقا ✨♥\n~ او تواصل مع المبرمج ويفي @oYsYY**", reply_to_message_id=message_id)
                    ...
                    await client.send_message(chat_id, f"**~ حدث خطأ حاول مرا آخري لاحقا ✨♥\n~ او تواصل مع الدعم : @oYsYY**", reply_to_message_id=message_id)
        return join        
                 ...
                 if h:
                  try:
                   ...
                   Done = True
                      ...
                      await client.send_message(chat_id, "**~ قم بتشغيل المكالمة أولاً ⚠️**", reply_to_message_id=message_id)
        except AlreadyJoinedError:
             await client.send_message(chat_id, "**~ قم بإعادة تشغيل المكالمة ⚠️**", reply_to_message_id=message_id)
...
async def logs(bot_username, client, message):
  try:
   ...
   if message.chat.type == ChatType.CHANNEL:
     ...
     text = f"**تم تشغيل اغنيه في مجموعه ✨♥**\n\n**اسم المحموعه : {chat}**\n**ايدي المجموعة : {message.chat.id}**\n**بواسطة : {name}**\n\n**المشغل : {message.text}**"
   else:
     chat = f"[{message.chat.title}](t.me/{message.chat.username})" if message.chat.username else message.chat.title
     user = f"يوزر المستخدم : @{message.from_user.username}" if message.from_user.username else f"User Id : {message.from_user.id}"
     text = f"**تم تشغيل اغنيه في مجموعة ✨♥**\n\n**اسم المجموعة : {chat}**\n**ايدي المجموعة : {message.chat.id}**\n**بواسطة : {message.from_user.mention}**\n**{user}**\n\n**المشغل : {message.text}**"
...
async def aii(client: Client, message):
   ...
   try:
    ...
    rep = await message.reply_text("**~ جاري تشغيل اغنية عشوائية لك انتظر قد يستغرق بضع دقائق ✨♥**")
    try:
          call = await get_call(bot_username)
    except:
    ...
    chat_id = message.chat.id
    user_id = message.from_user.id if message.from_user else "oYsYY"
    videoid = None
    link = None
    ...
    button = [[InlineKeyboardButton(text=".~ 𝑬𝒏𝒅", callback_data=f"stop"), InlineKeyboardButton(text="𝑹𝒆𝒔𝒖𝒎𝒆", callback_data=f"resume"), InlineKeyboardButton(text="𝑷𝒂𝒖𝒔𝒆 ~.", callback_data=f"pause")], [InlineKeyboardButton(text="᥉᥆ᥙᖇᥴᥱ ᥉᥆ꪔ᥆", url=f"https://t.me/Wevy_Python")]]
    await message.reply_photo(photo=photo, caption=f"**~ تم التشغيل عشوائي 🔀 .\n\n~ الاسم : {title} ⚡ .\n~ المدة : {duration} ✅ .\n~ بواسطة : {req} 👤 .**", reply_markup=InlineKeyboardMarkup(button))
...
async def play(client: Client, message):
  ...
  gr = await get_group(bot_username)
  ch = await get_channel(bot_username)
  button = [[InlineKeyboardButton(text=".~ 𝑬𝒏𝒅", callback_data=f"stop"), InlineKeyboardButton(text="𝑹𝒆𝒔𝒖𝒎𝒆", callback_data=f"resume"), InlineKeyboardButton(text="𝑷𝒂𝒖𝒔𝒆 ~.", callback_data=f"pause")]]
  ...
  if not message.reply_to_message:
     ...
     if await is_served_call(client, message.chat.id):
         ...
         requester = chatname if Wevy.views else f"[{message.from_user.first_name}](tg://user?id={message.from_user.id})"
         if message.from_user:
          if message.from_user.photo:
           ...
           ahmed = await client.get_chat("oYsYY")
         ...
         await message.reply_photo(photo=photo, caption=f"**~ تم الاضافة لقائمة التشغيل : {position} \n\n~ الاسم : {title[:18]} ⚡ .\n~ المدة : {duration} ✅ .\n~ بواسطة : {requester} 👤 .**", reply_markup=InlineKeyboardMarkup(button))
         await logs(bot_username, client, message)
     else:
         chat_id = message.chat.id
         title = title.title()
         ...
         if message.from_user:
          ...
          photo = await client.download_media(ahmedphoto)
         photo = await gen_thumb(videoid, photo)
         await message.reply_photo(photo=photo, caption=f"**~ تم التشغيل الان ✨♥ .\n\n~ الاسم : {title[:18]} ⚡ .\n~ المدة : {duration} ✅ .\n~ بواسطة : {requester} 👤 .**", reply_markup=InlineKeyboardMarkup(button))
         await logs(bot_username, client, message)
     await rep.delete()
  else:
       ...
       photo = "Uploaded to https://telegra.ph/file/3b8d5a2d60165d160b96c.jpg"
