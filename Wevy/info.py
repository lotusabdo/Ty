...
from Wevy.Data import get_data
from googletrans import Translator
from Wevy.Data import (get_call, get_app, get_userbot, get_group, get_channel, must_join)
...
ahmed = "https://telegra.ph/file/3b8d5a2d60165d160b96c.jpg"
...
async def change_stream(bot_username, client, chat_id):
           try:
            ...
            button = [[InlineKeyboardButton(text=".~ 𝑬𝒏𝒅", callback_data=f"stop"), InlineKeyboardButton(text="𝑹𝒆𝒔𝒖𝒎𝒆", callback_data=f"resume"), InlineKeyboardButton(text="𝑷𝒂𝒖𝒔𝒆 ~.", callback_data=f"pause")], [InlineKeyboardButton(text=" 𝒂𝒅𝒅 𝒎𝒆 𝒕𝒐 𝒚𝒐𝒖𝒓 𝒈𝒓𝒐𝒖𝒑 𝒐𝒓 𝒄𝒉𝒂𝒏𝒏𝒆l", url=f"https://t.me/{bot_username}?startgroup=True")]]
            await app.send_photo(chat_id, photo=img, caption=f"**~ تم التشغيل بنجاح 🎧 .\n\n~ الاسم : {title} ⚡ .\n~ المدة : {dur} ✅ .\n~ بواسطة : {requester} 👤 .**", reply_markup=InlineKeyboardMarkup(button))
            try:
               os.remove(file_path)
               os.remove(img)
            except:
...
async def helper(bot_username):
   ...
   async def helperuser(client, update):
     if not update.chat.id in ass[bot_username]:
      await user.send_message(update.chat.id, f"**~ انا الحساب الخاص بتشغيل الاغاني لسورس سومو ✨♥\n**")
