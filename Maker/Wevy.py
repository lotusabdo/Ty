  ...
  for i in bots:
      ...
      try:
       if not i["bot_username"] in Done:
        ...
        try:
           await user.send_message(bot_username,"**~ تم تنصيب البوت على سورس سومو ميوزك بنجاح ✨♥\n~ واستخدامي حساب مساعد للبوت ⚡**")
        except:
           pass
        try:
...
async def turnon(client, message):
 if message.chat.username in OWNER:
  ...
  try:
   ...
   pass
  return await message.reply_text("**~ تم تشغيل جميع البوتات بنجاح ✨♥**",quote=True)

@app.on_message(filters.command(["تشغيل المجاني", "تعطيل المجاني"], ""))
async def bye(client, message):
    user = message.from_user.username
    if user in OWNER:
        global OFF
        text = message.text
        if text == "تشغيل المجاني":
            OFF = None
...
async def stratmaked(client, message):
  ...
  if OFF:
      if not message.chat.username in OWNER:
         return await message.reply_text(f"**~ الوضع المجاني معطل الان راسل المطور لتنصيب مدفوع ✨♥**")
  if message.chat.username in OWNER:
    kep = ReplyKeyboardMarkup([
["تحديث الصانع"],
["《صنع بوت》", "《حذف بوت》"],
["《استخراج جلسه》"],
["تشغيل المجاني", "تعطيل المجاني"],
["تشغيل جميع البوتات","البوتات المصنوعه"],
["فحص البوتات", "تنظيف البوتات"],
...
["اذاعه للمطورين","المكالمات النشطه"]], resize_keyboard=True)
    ...
    kep = ReplyKeyboardMarkup([
["《حذف بوت》", "《صنع بوت》"],
["《السورس》", "《استخراج جلسه》"]],resize_keyboard=True)
    await message.reply_text(f"**~ أهلاً بك {message.from_user.mention} ✨♥ \n~ في صانع ميوزك سومو 🔱**", reply_markup=kep,quote=True)
    
@app.on_message(filters.command(["《السورس》"], ""))
async def source(c: Client, m):
  mostafa ="""
╭──── • ◈ • ────╮
么 [᥉᥆ᥙᖇᥴᥱ ᥉᥆ꪔ᥆](t.me/WevyPython)
么  [ძᥱ᥎ ხ᥆ƚ](t.me/oYsYY)
╰──── • ◈ • ────╯
𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝙼𝚄𝚂𝙸𝙲 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼 🇵🇸
"""
...
async def cloner(app: app, message):
    ...
    if OFF:
      if not message.chat.username in OWNER:
         return await message.reply_text(f"**~ الوضع المجاني معطل الان راسل المطور لتنصيب مدفوع ✨♥ @oYsYY**")
    ...
    await bot.stop()
    if message.chat.username in OWNER:
       dev = await app.ask(message.chat.id, "**~ ارسل الان ايدي المطور 👨‍✈️ \n\n- اذا كنت تريد هذا الحساب ارسل انا**", timeout=200)
       if dev.text == "انا":
          dev = message.chat.id
       else:
          dev = int(dev.text)
...
async def delbot(client: app, message):
  ...
  if message.chat.username in OWNER:
   ...
   if not bot_username in list:
     return await message.reply_text("**~ البوت مش مصنوع قبل كدا ✨♥**")
   else:
    try:
   ...
   if not message.chat.id in list:
     return await message.reply_text("**~ انت مش منصب قبل كدا ✨♥**")
   else:
    try:
     dev = message.chat.id
     dev = {"dev": dev}
...
@app.on_message(filters.command(["تنظيف البوتات"],""))
