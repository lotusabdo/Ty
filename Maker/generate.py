...
ask_ques = "**مرحبا بك في قسم استخراج جلسة لتنصيب سورس سومو ميوزك ⚡\n\n~ اضغط علي زر بيروجرام ✨♥**"
buttons_ques = [
    [
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if api_id_msg.text == "/skip":
        ...
        try:
            ...
            return
        api_hash_msg = await bot.ask(user_id,"**ارسل api_hash الخاص بالحساب \n\n~ اذا كنت تريد الاستخراج برقم الهاتف فقط ارسل /skip**", filters=filters.text)
    ...
    try:
        ...
        await msg.reply("**~ لقد تاخرت في ارسال الكود ⚡\n~ اعد محاولة استخراج الجلسة ✨♥**", reply_markup=InlineKeyboardMarkup(gen_button))
        return
    if not is_bot:
        ...
        try:
            ...
            try:
                two_step_msg = await bot.ask(user_id, "~ الحساب محمي بالتحقق بخطوتين .. ارسل باسورد الحساب ✨♥.**", filters=filters.text, timeout=300)
            except TimeoutError:
                await msg.reply("» لقد تاخرت في ارسال باسورد التحقق.\n\nقم باستخراج جلسه مره اخري ⚡", reply_markup=InlineKeyboardMarkup(gen_button))
                return
            try:
